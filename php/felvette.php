...
<form method="POST" action="hallgatokurzus.php" accept-charset="utf-8">
	<label>Válassz hallgatót!</label>
	<select name="vhallgato">
	<?php
		$hallgatok = hallgatolistatLeker();
		if(mysqli_num_rows($hallgatok) > 0){
			while( $sor = mysqli_fetch_assoc($hallgatok)){
				echo '<option value="'.$sor["neptun_kod"].'">'.$sor["neptun_kod"].' - '.$sor["nev"].'</option>';
			}
		} else {
			echo '<option value=""> NINCS HALLGATÓ </option>';
		}
		mysqli_free_result($hallgatok);
	?>
	</select>
	<br>
	<input type="submit" name="hallgatokurzus" value="Kurzusok listázása">
</form>
